  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const loop = async () => {
        ...
        for (let i = 0; i < 2; i++) {
          const h = hands[i];
          if (!h) { trails.current[i] = []; continue; }
          seen[i] = true;
          const kpts = h.keypoints;
          const x = (kpts[8].x + kpts[9].x + kpts[12].x) / 3;
          const y = (kpts[8].y + kpts[9].y + kpts[12].y) / 3;
          const tr = trails.current[i]; tr.push({ x, y }); if (tr.length > TRAIL_LEN) tr.shift();
          if (showOverlay.current) {
            ctx.save(); ctx.translate(W,0); ctx.scale(-1,1);
            kpts.forEach(pt => { ctx.beginPath(); ctx.arc(pt.x, pt.y, 4, 0, 2*Math.PI); ctx.fillStyle='lime'; ctx.fill(); });
            ctx.restore();
            ctx.save(); ctx.translate(W,0); ctx.scale(-1,1);
            ctx.strokeStyle = COLORS[i]; ctx.lineWidth = 2; ctx.beginPath();
            tr.forEach((p,j) => j ? ctx.lineTo(p.x,p.y) : ctx.moveTo(p.x,p.y)); ctx.stroke(); ctx.restore();
          }
          const segs = tr.slice(1).map((p,j) => { const dx = p.x - tr[j].x, dy = p.y - tr[j].y; return { d: Math.hypot(dx,dy), a: Math.atan2(dy,dx) }; }).filter(s => s.d >= MOVE_EPS);
          const total = segs.reduce((s,o) => s + o.d, 0) / iodScale;
          const speed = total / (tr.length/30);
          let turns = 0; segs.forEach((s,j) => { if(j>0){ let dA = Math.abs(s.a - segs[j-1].a); if(dA>Math.PI) dA = 2*Math.PI - dA; if(dA>ANGLE_THR) turns++; }});
